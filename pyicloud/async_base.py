...
class AsyncPyiCloudService:
    """
    An async authentication class for the iCloud service. Handles the
    ...
    Usage:
        ...
        async def main():
            api = await AsyncPyiCloudService.create('username@apple.com', 'password')
            # Use api...
            await api.close()
            
        # Or use as context manager:
        async with await AsyncPyiCloudService.create('username@apple.com', 'password') as api:
            # Use api...
    """
    ...
    def __init__(
        ...
        """
        Initialize AsyncPyiCloudService. Use create() class method instead.
        
        This __init__ should not be called directly. Use the create() class method
        which properly handles async initialization.
        """
        ...
        self._session: AsyncPyiCloudSession = AsyncPyiCloudSession(
            self,
            verify=verify,
            headers={
        ...
        # Service instances will be created lazily
        self._account = None
        self._calendar = None
        self._contacts = None
        self._devices = None
    ...
    async def create(
        ...
        """
        Create and initialize an AsyncPyiCloudService instance.
        
        This is the recommended way to create an AsyncPyiCloudService instance
        as it handles async initialization properly.
        """
        instance = cls(
            apple_id=apple_id,
            password=password,
    ...
    async def authenticate(
        ...
        if self.session.data.get("session_token") and not force_refresh:
            try:
                self.data = await self._validate_token()
        ...
        if (
            ...
            if "canLaunchWithOneFactor" in app and app["canLaunchWithOneFactor"]:
                ...
                try:
                    await self._authenticate_with_credentials_service(service)
                    login_successful = True
    ...
    async def _handle_accept_terms(self, login_data: dict) -> None:
        ...
        if self.data.get("termsUpdateNeeded"):
            ...
            resp: httpx.Response = await self.session.get(
    ...
    async def _authenticate(self) -> None:
        ...
        try:
            ...
            await self._srp_authentication(headers)
            await self._authenticate_with_token()

    async def _srp_authentication(self, headers: dict[str, Any]) -> None:
        """SRP authentication."""
        if self._password_raw is None:
        ...
        try:
            ...
            response: httpx.Response = await self.session.post(
        ...
        try:
            ...
            self._auth_data = await self._get_mfa_auth_options()
    ...
    async def _authenticate_with_token(self) -> None:
        ...
        try:
            ...
            await self._handle_accept_terms(login_data)

            if not self.is_trusted_session:
                raise PyiCloud2FARequiredException(self.account_name, resp)
        except (PyiCloudAPIResponseException, httpx.HTTPStatusError) as error:
            msg = "Invalid authentication token."
            raise PyiCloudFailedLoginException(msg, error) from error

    async def _authenticate_with_credentials_service(
    ...
    async def _validate_token(self) -> Any:
        ...
        try:
            req: httpx.Response = await self.session.post(
    ...
    async def get_trusted_devices(self) -> list[dict[str, Any]]:
        """Returns devices trusted for two-step authentication."""
        request: httpx.Response = await self.session.get(
            f"{self._setup_endpoint}/listDevices", params=self.params
        )
        return request.json().get("devices")

    async def send_verification_code(self, device: dict[str, Any]) -> bool:
        """Requests that a verification code is sent to the given device."""
        request: httpx.Response = await self.session.post(
    ...
    async def validate_verification_code(
        ...
        await self.trust_session()

        return not self.requires_2sa

    async def _get_mfa_auth_options(self) -> Dict:
        """Retrieve auth request options for assertion."""
        headers = self._get_auth_headers({"Accept": CONTENT_TYPE_JSON})

        resp = await self.session.get(self._auth_endpoint, headers=headers)
        return resp.json()
    ...
    async def _submit_webauthn_assertion_response(self, data: Dict) -> None:
    ...
    async def confirm_security_key(
        self, device: Optional[CtapHidDevice] = None
    ) -> None:
        ...
        await self._submit_webauthn_assertion_response(
            {
                "challenge": challenge,
    ...
    async def _check_pcs_consent(self) -> dict[str, Any]:
    ...
    async def _request_pcs_for_service(self, app_name: str) -> None:
        """Request PCS access for a specific service."""
        _check_pcs_resp: dict[str, Any] = await self._check_pcs_consent()
        ...
        if not _check_pcs_resp.get("isDeviceConsentedForPCS", True):
            ...
            if not resp_data.get("isDeviceConsentNotificationSent"):
        ...
        for _ in range(PCS_MAX_RETRIES):
            ...
            await asyncio.sleep(PCS_SLEEP_TIME)
            _check_pcs_resp = await self._check_pcs_consent()

        for attempt in range(PCS_MAX_RETRIES):
            resp: dict[str, Any] = await self._send_pcs_request(
                app_name,
    ...
    async def validate_2fa_code(self, code: str) -> bool:
        """Verifies a verification code received via Apple's 2FA system (HSA2)."""
        try:
            if self._auth_data.get("mode") == "sms":
                await self._validate_sms_code(code)
            else:
    ...
    async def _validate_sms_code(self, code: str) -> None:
    ...
    async def trust_session(self) -> bool:
    ...
    def __str__(self) -> str:
        return f"Async iCloud API: {self.account_name}"
